...
tags:
  - detail-note
---
함수나 컴포넌트의 이름, 파라미터, 반환 값에 드러나지 않는 숨은 로직이 있다면, 함께 협업하는 동료들이 동작을 예측하는 데에 어려움을 겪을 수 있다

### 코드 살펴보기
---
다음 코드는 사용자의 계좌 잔액을 조회할 때 사용할 수 있는 `fetchBalance` 함수이다
함수를 호출할 때마다 암시적으로 `balance_fetched`라는 로깅이 이루어지고 있다
...
- **예측 가능성**
	`fetchBalance` 함수의 이름과 반환 타입만 갖고서는 `balance_fetched` 라는 로깅이 이루어지는지 알 수 없다
