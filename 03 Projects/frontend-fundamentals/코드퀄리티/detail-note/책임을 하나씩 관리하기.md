...
tags:
  - detail-note
---
쿼리 파라미터, 상태, API 호출과 같은 로직의 종류에 따라서 함수나 컴포넌트, Hook을 나누지 마라
...
- ** 결합도**:  이 Hook은 `"이 페이지에 필요한 모든 쿼리 매개변수를 관리하는 것"`이라는 광범위한 책임을 가지고 있다
	
	이로 인해 페이지 내의 컴포넌트나 다른 훅들이 이 훅에 의존하게 될 수 있으며, 코드 수정을 할 때 영향 범위가 급격히 확장될 수 있다

	시간이 지나며 이 Hook은 유지 관리가 점점 어려워지고, 수정하기 힘든 코드로 발전할 수 있다



### 코드 개선하기
---
다음 코드와 같이 각각의 쿼리 파라미터별로 별도의 Hook을 작성할 수 있다
