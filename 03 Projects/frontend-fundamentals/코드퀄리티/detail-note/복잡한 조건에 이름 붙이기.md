...
tags:
  ...
  - 코드퀄리티
---
복잡한 조건식이 특별한 이름없이 사용되면, 조건을 뜻하는 바를 한눈에 파악하기 어렵다
...
- 가독성
	...
	`filter`와 `some`, `&&` 같은 로직이 여러 단계로 중첩되어 있어서 정확한 조건을 파악하기 어렵다

### 코드 개선하기
---
조건에 `isSameCategory`, `isPriceInRange`와 같이 명시적인 이름을 붙이면, 코드를 읽는 사람이 한 번에 고려해야 할 맥락을 줄일 수 있다
